...
//    pcl::KdTreeFLANN<PointType>::Ptr kdtree_static_global_curr_;
    pcl::KdTreeFLANN<PointType> kdtree_dyna_global_curr_;
    pcl::KdTreeFLANN<PointType> kdtree_static_global_curr_;
...
public:
    ...
    pcl::PointCloud<PointType>::Ptr map_global_static; // 最终的所有的静态点，由 一开始的地面点 + 后面remove操作的剩下的静态点 + revert操作恢复的静态点
    pcl::PointCloud<PointType>::Ptr map_subset_global_curr_;

    std::vector<std::vector<int>> all_scan_dynamic_idx;
    std::vector<std::vector<int>> all_scan_static_idx;
    ...
    std::pair<cv::Mat, cv::Mat> scan2RangeImg(const pcl::PointCloud<PointType>::Ptr& _scan,
    ...
    std::pair<std::vector<int>,std::vector<int>> calcDescrepancyAndParseDynamicPointIdx (const cv::Mat& _scan_rimg, const cv::Mat& _diff_rimg, const cv::Mat& _map_rimg_ptidx,const cv::Mat & _rimg_Scan_ptidx);
    ...
    std::vector<Eigen::Matrix3f> PCAEachcluster(std::vector<pcl::PointCloud<PointType>::Ptr> vector_dynamic_static_Ptr);
    void revertDynamicPointsByKnnNew();
