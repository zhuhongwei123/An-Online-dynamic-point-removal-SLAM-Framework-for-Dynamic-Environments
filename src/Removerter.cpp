...
#include "removert/PointsCanopyCluster.h"
...
Removerter::Removerter() {
    ...
    nh.param<int>("removert/num_nn_points_within", kNumKnnPointsToCompare, 100); // using higher, more strict static
...
std::pair<cv::Mat, cv::Mat> Removerter::scan2RangeImg(const pcl::PointCloud<PointType>::Ptr &_scan,
                                  const std::pair<float, float> _fov, /* e.g., [vfov = 50 (upper 25, lower 25), hfov = 360] */
    ...
    cv::Mat rimg_Scan_ptidx = cv::Mat(kNumRimgRow, kNumRimgCol, CV_32SC1,
                                 cv::Scalar::all(0)); // int matrix, save point (of global map) index
...
//            cv::imwrite("/home/zhuhongwei/Removert_ws/src/removert/zhuhongweiTest/scan_rimg" + std::to_string(number)+ ".jpg",rimg_color_Scans);
            rimg.at<float>(pixel_idx_row, pixel_idx_col) = curr_range;
            rimg_Scan_ptidx.at<int>(pixel_idx_row, pixel_idx_col) = pt_idx;
        }
    }

    return  std::pair<cv::Mat, cv::Mat>(rimg, rimg_Scan_ptidx) ;
} // scan2RangeImg
...
//    map_global_curr_ = map_global_orig_;
...
void Removerter::saveFinalStaticAndDynamicPointCloudGlobal() {
    ...
    pcl::io::savePCDFileBinary(static_file_name_final + "/finalStaticOur.pcd", *map_global_static);
    ROS_INFO_STREAM(
            "\033[1;32m -- a pointcloud is saved: " << static_file_name_final + "/finalStaticOur.pcd" << "\033[0m");

    pcl::io::savePCDFileBinary(static_file_name_final + "/finalStaticRemovert.pcd", *map_global_curr_);
    ROS_INFO_STREAM(
            "\033[1;32m -- a pointcloud is saved: " << static_file_name_final + "/finalStaticRemovert.pcd" << "\033[0m");


}

std::pair<std::vector<int>,std::vector<int>> Removerter::calcDescrepancyAndParseDynamicPointIdx
        (const cv::Mat &_scan_rimg, const cv::Mat &_diff_rimg, const cv::Mat &_map_rimg_ptidx,const cv::Mat & rimg_Scan_ptidx) {
    int num_dyna_points{
    ...
    std::vector<int> dynamic_point_indexes_map;
    std::vector<int> dynamic_point_indexes_scan;
    for (int row_idx = 0; row_idx < _diff_rimg.rows; row_idx++) {
        for (int col_idx = 0; col_idx < _diff_rimg.cols; col_idx++) {
            ...
            /// 自适应系数，后面可以调整的
            float adaptive_coeff = 0.5; // meter, // i.e., if 4m apart point, it should be 0.4m be diff (nearer) wrt the query
            ...
            if (this_diff <
                ...
                int this_point_idx_in_scan = rimg_Scan_ptidx.at<int>(row_idx, col_idx);
                dynamic_point_indexes_map.emplace_back(this_point_idx_in_global_map);
                dynamic_point_indexes_scan.emplace_back(this_point_idx_in_scan);

            }
        }
    }

    return std::pair<std::vector<int>,std::vector<int>>(dynamic_point_indexes_map,dynamic_point_indexes_scan);
 ...
 * removert revert
 * */
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdxForEachScanForRevert(std::pair<int, int> _rimg_shape) {
    ...
    for (std::size_t idx_scan = 0; idx_scan < scans_.size(); ++idx_scan) {
        ...
        auto[scan_rimg, rimg_Scan_ptidx]  = scan2RangeImg(_scan, kFOV, _rimg_shape, idx_scan); // openMP inside
        ...
        auto [this_scan_dynamic_point_mapindexes,dynamic_point_indexes_scan] = calcDescrepancyAndParseDynamicPointIdx(scan_rimg, diff_rimg, map_rimg_ptidx,rimg_Scan_ptidx);

        dynamic_point_indexes.insert(dynamic_point_indexes.end(), this_scan_dynamic_point_mapindexes.begin(),
                                     this_scan_dynamic_point_mapindexes.end());
...
//        RemoveFormMatScansSavePNG(scan_rimg, kRangeColorAxis, idx_scan);
        ...
        /// 把地图转换成深度图片
        auto[map_rimg, map_rimg_ptidx] = map2RangeImg(map_local_curr_, kFOV, _rimg_shape,  idx_scan); // the most time comsuming part 2 -> so openMP applied inside
...
//        RemoveForDiffMAPSavePNG(diff_rimg, kRangeColorAxis, idx_scan);
        ...
        // 这一帧的动态点的索引放入 all_scan_dynamic_idx
        all_scan_dynamic_idx.push_back(dynamic_point_indexes_scan);
        std::vector<int> static_point_indexes_scan = getGlobalMapStaticIdxFromDynamicIdx(_scan, dynamic_point_indexes_scan);
        all_scan_static_idx.push_back(static_point_indexes_scan);

        /// 每一帧的动态点都加入到这个向量里，这样往里面叉数据，肯定会有动态点云的重复问题，因此，下文会对于正好向量进行set去重
        dynamic_point_indexes.insert(dynamic_point_indexes.end(), this_scan_dynamic_point_mapindexes.begin(), this_scan_dynamic_point_mapindexes.end());
...
std::vector<int> Removerter::calcDescrepancyAndParseDynamicPointIdxForBatchScan(std::pair<int, int> _rimg_shape) {
    ...
    for (std::size_t idx_scan = 0; idx_scan < size_scans - 1; ++idx_scan) {
        ...
        ROS_INFO_STREAM("\033[1;32m -- ... have been solved" << (idx_scan) << " number" "\033[0m");
        // scan's pointcloud to range img ，生成深度图，_scan是在激光雷达坐标系下的
        auto[scan_rimg, rimg_Scan_ptidx] = scan2RangeImg(BatchScan, kFOV, _rimg_shape, idx_scan); // openMP inside
        RemoveFormMatScansSavePNG(scan_rimg, kRangeColorAxis, idx_scan);
        // map's pointcloud to range img
        if (kUseSubsetMapCloud) {
            takeGlobalMapSubsetWithinBall(idx_scan);
        ...
        /// 每一帧的动态点都加入到这个向量里，这样往里面叉数据，肯定会有动态点云的重复问题，因此，下文会对于正好向量进行set去重
        all_scan_dynamic_idx.push_back(dynamic_point_indexes_scan);
        std::vector<int> static_point_indexes_scan = getGlobalMapStaticIdxFromDynamicIdx(BatchScan, dynamic_point_indexes_scan);
        all_scan_static_idx.push_back(static_point_indexes_scan);
...
void Removerter:: revertOnce( float _res_alpha )
{
    ...
    ROS_INFO_STREAM("\033[1;32m -- The range image size is: [" << rimg_shape.first << ", " << rimg_shape.second << "].\033[0m");
    ROS_INFO_STREAM("\033[1;32m -- The number of map points: " << map_global_curr_dynamic_all_res->points.size() << "\033[0m");
    /// revert 的主函数
    std::vector<int> dynamic_point_indexes = calcDescrepancyAndParseDynamicPointIdxForEachScanForRevert(rimg_shape);
    ROS_INFO_STREAM("\033[1;32m -- The number of dynamic points: " << dynamic_point_indexes.size() << "\033[0m");
    /// 获得动态点的地图
    parseDynamicMapPointcloudUsingPtIdx(map_global_curr_dynamic_all_res, dynamic_point_indexes,map_global_curr_dynamic_);

    std::vector<int> static_point_indexes = getGlobalMapStaticIdxFromDynamicIdx(map_global_curr_dynamic_all_res, dynamic_point_indexes);
    ROS_INFO_STREAM("\033[1;32m -- The number of static points: " << static_point_indexes.size() << "\033[0m");
    /// 获得静态点地图
...
std::vector<Eigen::Matrix3f> Removerter::PCAEachcluster(std::vector<pcl::PointCloud<PointType>::Ptr> vector_dynamic_static_Ptr) {

    std::vector<Eigen::Matrix3f> vector_eigenVectorsPCA;
    for (auto cloud: vector_dynamic_static_Ptr) {
        ...
        vector_eigenVectorsPCA.push_back(eigenVectorsPCA);
    }
    return vector_eigenVectorsPCA;
};


void Removerter::revertDynamicPointsByKnnNew() {

    kdtree_static_global_curr_.setInputCloud(map_global_static);
    int num_points_of_static = map_global_curr_dynamic_all_res->points.size();

    kdtree_dyna_global_curr_.setInputCloud(map_global_curr_dynamic_all_res);
    int num_points_of_dynamic = map_global_curr_dynamic_all_res->points.size();

    pcl::PointCloud<PointType>::Ptr static_global(new pcl::PointCloud<PointType>);
    pcl::PointCloud<PointType>::Ptr dynamic_global(new pcl::PointCloud<PointType>);

    pcl::ExtractIndices<PointType> extractor_dyna;
    pcl::PointCloud<PointType>::Ptr output_dynamic;
    extractor_dyna.setInputCloud(map_global_curr_dynamic_all_res);

    pcl::PointCloud<PointType>::Ptr output_static;
    pcl::ExtractIndices<PointType> extractor_static;
    extractor_static.setInputCloud(map_global_static);

    ROS_INFO_STREAM("\033[1;32m --start KNN**************"  << "\033[0m");
    for (std::size_t pt_idx = 0; pt_idx < num_points_of_dynamic; pt_idx++){
        std::vector<int> topk_indexes_dyna;
        std::vector<float> topk_L2dists_dyna;
        std::vector<int> topk_indexes_static;
        std::vector<float> topk_L2dists_static;

        kdtree_dyna_global_curr_.nearestKSearch(map_global_curr_dynamic_all_res->points[pt_idx], kNumKnnPointsToCompare,
                                                 topk_indexes_dyna, topk_L2dists_dyna);

        kdtree_static_global_curr_.nearestKSearch(map_global_curr_dynamic_all_res->points[pt_idx], kNumKnnPointsToCompare,
                                                   topk_indexes_static, topk_L2dists_static);

        boost::shared_ptr<std::vector<int>> index_dyna_ptr = boost::make_shared<std::vector<int>>(topk_indexes_dyna);
        extractor_dyna.setIndices(index_dyna_ptr);
        extractor_dyna.setNegative(false); // If set to true, you can extract point clouds outside the specified index
        output_dynamic->clear();
        extractor_dyna.filter(*output_dynamic);

        boost::shared_ptr<std::vector<int>> index_ptr_static = boost::make_shared<std::vector<int>>(topk_indexes_static);
        extractor_static.setIndices(index_ptr_static);
        extractor_static.setNegative(false); // If set to true, you can extract point clouds outside the specified index
        output_static->clear();
        extractor_static.filter(*output_static);

        std::vector<pcl::PointCloud<PointType>::Ptr> vector_dynamic_static_Ptr;
        vector_dynamic_static_Ptr.emplace_back(output_dynamic);
        vector_dynamic_static_Ptr.emplace_back(output_static);
        std::vector<Eigen::Matrix3f> vector_eigenVectorsPCA = PCAEachcluster(vector_dynamic_static_Ptr);

//        eigenVectors_compare(vector_eigenVectorsPCA);
    }
...
//    FilterBEV.RunLidar2BEVAndRemove();
...
//    revertDynamicPointsByKnnNew();
//
//  clustering method for revert, our previous work
//    MainProcessCanopy(map_global_curr_dynamic_all_res,0.2,0.1);
//
    *map_global_static += *map_global_curr_;
