...
void CloudFilter::removeFloor(double cell_size, double height_threshold, int grid_dim,Eigen::Matrix4d scan_pose) {
    ...
    /// Avoid pcl filters if possible because they copy the cloud inside.Points can be removed using std algorithms (erase + remove_if)
//    cloud_ptr_->points.erase(std::remove_if(cloud_ptr_->points.begin(),
//                                            cloud_ptr_->points.end(),
//                                            [&](const pcl::PointXYZI &p) {
...
//    NonGroundScanPoints.push_back(cloud_ptr_);
    ...
    for (int pt_idx = 0; pt_idx < cloud_ptr_->points.size(); ++pt_idx) {
        bool PointofGround = filterGround(cloud_ptr_->points[pt_idx], grid_dim, min, max, init, height_threshold,cell_size);
        if(PointofGround) {
            GroundIndex.push_back(pt_idx);
        }
    }
    ...
    boost::shared_ptr<std::vector<int>> index_ptr = boost::make_shared<std::vector<int>>(GroundIndex);
...
//float CloudFilter::rad2deg(float radians) {
...
//            cloud_ptr_->points.size())));
    ...
    cv::Mat image(kNumRimgRow, kNumRimgCol, CV_8UC3, cv::Scalar(255, 255, 255));
...
//        ROS_INFO_STREAM("\033[1;32m --  process" << pt_idx << "\033[0m");
        ...
        if(this_point.x <50 && this_point.x>-50 && this_point.y <50 && this_point.y>-50){
            ...
            float el_deg = sph_point.el * 180.0 / M_PI;
            float az_deg = sph_point.az * 180.0 / M_PI;

            auto tmp = kNumRimgRow * (1 - (el_deg+ (kVFOV / float(2.0))) / (kVFOV - float(0.0)));
            auto val = std::round(tmp);
            int pixel_idx_row = int(std::min(std::max(val,float(lower_bound_row_idx)), float(upper_bound_row_idx)));

            int pixel_idx_col = int(std::min(std::max(
                    std::round(kNumRimgCol * ((az_deg + (kHFOV / float(2.0))) / (kHFOV - float(0.0)))),
                    float(lower_bound_col_idx)), float(upper_bound_col_idx)));

            image.at<uchar>(pixel_idx_row,pixel_idx_col) = std::min(int(this_point.z), 255);

            if (!init[pixel_idx_row][pixel_idx_col]) {
                min[pixel_idx_row][pixel_idx_col] = this_point.z;
                max[pixel_idx_row][pixel_idx_col] = this_point.z;
        ...
        for (int i = 0; i <= kNumRimgRow-1; ++i) {
            ...
            {
                if (init[i][j]){
                    if (max[i][j] - min[i][j] < HeightThreshold && max[i][j]<-0.4 &&  min[i][j]>-2) {
...
//        std::string ground_file_name = save_pcd_directory_ + "/Ground.pcd";
//        ROS_INFO_STREAM("\033[1;32m save ground map" << ground_file_name << "\033[0m");
//        pcl::io::savePCDFileBinary(ground_file_name, *(RemoverterPtr->map_global_static));

//        cv::imwrite("/home/zhuhongwei/Removert_sci4/src/removert/src/TEST.PNG",image);
}
 ...
 */
void CloudFilter::RunLidar2BEVAndRemove() {
    ROS_INFO_STREAM("\033[1;32m -- start change to BEV ********" "\033[0m");
    RemoverterPtr->map_global_curr_->clear();
    RemoverterPtr->map_local_curr_->clear();
    RemoverterPtr->map_global_static->clear();
...
//        removeFloor(CellSize, HeightThreshold, GridDim,scan_pose);
