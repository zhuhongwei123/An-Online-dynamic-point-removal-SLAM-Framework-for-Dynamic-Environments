...
: nh(nh_super), ROSimg_transporter_(nh)
{
    ...
    // fov 
    nh.param<float>("removert/sequence_vfov", kVFOV, 30.0);
    nh.param<float>("removert/sequence_hfov", kHFOV, 360.0);
    kFOV = std::pair<float, float>(kVFOV, kHFOV);

    // resolution 
    nh.param<std::vector<float>>("removert/remove_resolution_list", remove_resolution_list_, std::vector<float>{0.5}); // 分辨率列表
    nh.param<std::vector<float>>("removert/revert_resolution_list", revert_resolution_list_, std::vector<float>{1.2, 0.5});
    ...
    nh.param<int>("removert/end_idx", end_idx_, 5);
    ...
    nh.param<std::string>("removert/save_pcd_directory", save_pcd_directory_, "/home/zhuhongwei/Removert_sci4/src/removert/PCDData/KITTY/test08_2");
// ransac seg
...
// Lidar2BEV
    ...
    nh.param<int>("Lidar2BEVAndRemovePara/grid_dim", GridDim, 1000);
    ...
    nh.param<int>("Lidar2BEVAndRemovePara/grid_dim_height_map", GridDimHeightMap, 300);//300*cell_size = total pointcloud size
